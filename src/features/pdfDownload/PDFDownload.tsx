...
export const PDFDownload: FC<PDFDownloadProps> = ({ className, Template, filename = 'tables' }) => {
  return (
    <PDFDownloadLink
      ...
      className={cl(className, styles['pdf-btn'], 'ant-btn-primary')}
    >
      {({ loading }) =>
        loading ? (
          <Spin size='small' className='pdf-btn__spin' />
        ) : (
          <span className={cl(styles['pdf-btn__icon'])} title='Сохранить в PDF'>
